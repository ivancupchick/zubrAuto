...
export const getFieldChainsValue = (
  ...
  fields.forEach((f) => {
    if (f.type === FieldType.Dropdown || f.type === FieldType.Multiselect) {
      const tempValue = Array.isArray(query[f.name])
        ? query[f.name][0]
        : query[f.name];
      ...
      const needVariants = Array.isArray(query[f.name])
        ? (query[f.name] as unknown as string[])
        : query[f.name].split(',');
      query[f.name] = needVariants
        .map((v) => {
          if (f.variants) {
            const index = f.variants
              .split(',')
              .findIndex((vValue) => vValue === v);
  ...
  const queryValues = fields
    .map((f) => f.name)
    .map((k) =>
      Array.isArray(query[k])
        ? (query[k] as unknown as string[])
        : query[k].split(','),
    );
...
export const getFieldsWithValues = (
  chainedFields: Models.Field[],
  chaines: Models.FieldChain[],
  sourceId: number,
): RealField.Response[] => {
  return (
    chainedFields
      ...
      .map((cf) => {
        return {
          ...
          value:
            chaines.find((c) => c.fieldId === cf.id && c.sourceId === sourceId)
              ?.value || '',
        };
      })
  );
...
export class FieldsUtils {
  ...
  static getFields(
    entityOrFieldsArray:
      ...
      | Pick<RealField.Response, 'name' | 'value'>[],
  ): Pick<RealField.Response, 'name' | 'value'>[] {
  ...
  static getField(
    entityOrFieldsArray:
      ...
      | Pick<RealField.Response, 'name' | 'value'>[],
    name: string,
  ): Pick<RealField.Response, 'name' | 'value'> | null {
