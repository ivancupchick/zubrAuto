...
export const getGetSortedAllQuery = (
  tableName: string,
  sortField: string,
  sortOrder: string,
) => {
...
export const getDeleteByAndExpressions = <T>(
  tableName: string,
  expressions: ExpressionHash<T>,
) => {
  return `DELETE FROM \`${tableName}\` WHERE (${Object.keys(expressions)
    .map(
      (key) => `${key} IN (${expressions[key].map((e) => `'${e}'`).join(',')})`,
    )
    .join(' AND ')});`;
};

// need type for object
export const getUpdateByIdQuery = <T>(
  tableName: string,
  id: number,
  ...
  Object.keys(entity)
    .filter((key) => (isField ? key !== 'id' && key !== 'value' : key !== 'id'))
    .forEach((key) => (entityHash[key] = `${entity[key]}`));
...
// need type for object
export const getUpdateByAndExpressionQuery = <T>(
  tableName: string,
  entity: Partial<Omit<T, 'id'>>,
  expressions: ExpressionHash<Omit<T, 'id'>>,
  isField: boolean = false,
) => {
  // TODO add Object.keys(entity).reduce
  const entityHash: StringHash = {};
  ...
  return `UPDATE \`${tableName}\` SET ${Object.keys(entityHash)
    .map((key) => `${key} = '${entityHash[key]}'`)
    .join(',')} WHERE (${Object.keys(expressions)
    .map(
      (key) => `${key} IN (${expressions[key].map((e) => `'${e}'`).join(',')})`,
    )
    .join(' AND ')});`;
...
export function getInsertOneQuery<T>(tableName: string, entity: Omit<T, 'id'>) {
  ...
  const keys: string[] = Object.keys(entityHash).map((key) => `${key}`);
  const values: string[] = Object.keys(entityHash).map(
    (key) => `'${entityHash[key]}'`,
  );
...
export const getGetAllByOneColumnExpressionQuery = <T>(
  tableName: string,
  expressions: ExpressionHash<T>,
) => {
  return getGetAllByExpressionAndQuery(tableName, expressions);
...
export const getGetAllByExpressionAndQuery = <T>(
  tableName: string,
  expressions: ExpressionHash<T>,
) => {
  ...
  // }, {})
  return `SELECT * FROM \`${tableName}\` WHERE (${Object.keys(expressions)
    .map(
      (key) => `${key} IN (${expressions[key].map((e) => `'${e}'`).join(',')})`,
    )
    .join(' AND ')});`;
...
export const getGetSortedAllByExpressionAndQuery = <T>(
  tableName: string,
  expressions: ExpressionHash<T>,
  sortField: string,
  sortOrder: string,
  ...
  return `SELECT * FROM \`${tableName}\` WHERE (${Object.keys(expressions)
    ...
    .join(' AND ')}) ORDER BY ${sortField} ${sortOrder};`;
};

export const getGetAllByExpressionOrQuery = <T>(
  tableName: string,
  expressions: ExpressionHash<T>,
) => {
  return `SELECT * FROM \`${tableName}\` WHERE (${Object.keys(expressions)
