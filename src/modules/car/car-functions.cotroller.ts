...
export class CarFunctionsController {
  // TODO think about anatoher name or structure
  constructor(
    private readonly carService: CarService,
    private readonly carStatisticService: CarStatisticService,
  ...
  @Post(
    `/${Constants.API.STATISTIC}/${Constants.API.ADD_CUSTOMER_DISCOUNT}/:carId`,
  ) // TODO validation
  async addCustomerDiscount(
    @Param() params: any,
    @Body() body: AddCustomerDiscountDto,
  ) {
    return await this.carStatisticService.addCustomerDiscount(
      +params.carId,
      body.discount,
      body.amount,
    );
  ...
  async createCarShowing(
    @Param() params: any,
    @Body() body: { showingContent: CarStatistic.ShowingContent },
  ) {
    return await this.carStatisticService.createCarShowing(
      +params.carId,
      body.showingContent,
    );
  }
  ...
  async updateCarShowing(
    @Param() params: any,
    @Body()
    body: { showingId: number; showingContent: CarStatistic.ShowingContent },
  ) {
    return await this.carStatisticService.updateCarShowing(
      body.showingId,
      +params.carId,
      body.showingContent,
    );
  ...
  async deleteCarImage(@Param() params: any) {
    return await this.carImageService.deleteCarImage(
      +params.carId,
      +params.imageId,
    );
  }
  ...
  async uploadCarImages(
    @Body() body: { carId: number; metadata: string },
    ...
    if (uploadedFile) {
      files = Array.isArray(uploadedFile) ? uploadedFile : [uploadedFile];
    }
    ...
    return await this.carImageService.uploadCarImages(
      +body.carId,
      files,
      body.metadata || '{}',
    );
  ...
  async uploadCarStateImages(
    ...
    return await this.carImageService.uploadCarStateImages(
      +body.carId,
      files,
      body.metadata || '{}',
    );
  ...
  async uploadCarImage360(
    ...
    return await this.carImageService.uploadCarImage360(
      +body.carId,
      files[0],
      body.metadata || '{}',
    );
