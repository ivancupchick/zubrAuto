...
export class CarImageService {
  ...
  async uploadCarImages(carId: number, files: Express.Multer.File[], fileMetadata: string): Promise<any> {
  ...
  async uploadCarStateImages(carId: number, files: Express.Multer.File[], fileMetadata: string): Promise<any> {
    // const extension = file.name.split('.').pop();
  ...
  async uploadCarImage360(carId: number, file: Express.Multer.File, fileMetadata: string): Promise<any> {
    // const extension = file.name.split('.').pop();
    ...
    const allImageFileChains = await this.prisma.filesIds.findMany({ where: { sourceName: Models.Table.Cars, sourceId: carId }})
    const allImageFiles = await this.prisma.files.findMany();

    const existImage360 = allImageFiles.find(file => file.type === ServerFile.Types.Image360 && allImageFileChains.find(ch => ch.sourceId === carId)); // ch.sourceId === carId may not need?

    if (existImage360) {
      await this.deleteCarImage(carId, existImage360.id);
    }

    const sendData: ManagedUpload.SendData = await this.s3Service.uploadFile(file)
  ...
  async deleteCarImage(carId: number, imageId: number):Promise<ServerFile.IdResponse> {
    const result = await this.prisma.filesIds.deleteMany({where:{ fileId: imageId, sourceName: Models.Table.Cars, sourceId: carId }});
