...
export class ClientService {
  ...
  async findMany(query: StringHash) {
    ...
    const clients = clientsIds.map((id) =>
      clientsResults.find((cr) => +cr.id === +id),
    );
  ...
  async update(id: number, updateClientDto: UpdateClientDto) {
    ...
    const existsFields = updateClientDto.fields.filter((f) =>
      existFieldIds.includes(+f.id),
    );
    const nonExistFields = updateClientDto.fields.filter(
      (f) => !existFieldIds.includes(+f.id),
    );
  ...
  async completeDeal(clientId: number, carId: number) {
    ...
    const clientStatusField = clientFields.find(
      (cf) => cf.name === FieldNames.Client.dealStatus,
    );
    const carStatusField = carFields.find(
      (cf) => cf.name === FieldNames.Car.status,
    );
    ...
    const clientStatusIndex = clientStatusField.variants
      .split(',')
      .findIndex((v) => v === FieldNames.DealStatus.Sold);
    const carStatusIndex = carStatusField.variants
      .split(',')
      .findIndex((v) => v === FieldNames.CarStatus.customerService_Sold);
    ...
    const res = await Promise.all([
      this.fieldChainService.updateById(
        clientStatusChain.id,
        clientStatusChain.fieldId,
        { value: clientStatus },
      ),
      this.fieldChainService.updateById(
        carStatusChain.id,
