...
export class RoleService {
  ...
  async findAll() {
    const [roles, fieldAccesses] = await Promise.all([
      ...
      this.prisma.fieldAccesses.findMany({
        where: { sourceName: Models.Table.Roles },
      }),
    ]);
  ...
  async update(id: number, updateRoleDto: UpdateRoleDto) {
    ...
    const createdAccess: Models.FieldAccess[] =
      updateRoleDto.accesses.length > 0
        ...
        : [];
    const notCreatedAccess = updateRoleDto.accesses.filter((a) =>
      createdAccess.find((ca) => ca.fieldId === a.fieldId),
    );

    await Promise.all(
      (createdAccess || []).map((a) =>
        this.prisma.fieldAccesses.updateMany({
          data: {
            access: a.access,
          },
          where: {
  ...
  async remove(id: number) {
    const accesses = await this.prisma.fieldAccesses.findMany({
      where: {
        sourceId: id,
        sourceName: Models.Table.Roles,
      },
