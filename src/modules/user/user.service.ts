...
export class UserService {
  constructor(private prisma: PrismaService, private fieldChainService: FieldChainService, private fieldsService: FieldsService) {}
  ...
  async findMany(query: StringHash): Promise<BaseList<ServerUser.Response>> {
    ...
    const searchUsersIds = (Object.values(naturalQuery).length ? await this.fieldChainService.getEntityIdsByQuery(
      Models.Table.Users,
      FieldDomains.User,
      query
    ) : []).map(id => +id);
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    const user: Models.User = await this.prisma.users.create({data: createUserDto});
  ...
  async update(id: number, updateUserDto: UpdateUserDto) { // TODO deleting userTokens
  ...
  async remove(id: number) { // TODO deleting userTokens
  ...
  async findOne(id: number) {
    ...
    const relatedFields = await this.fieldsService.getFieldsByDomain(FieldDomains.User);
    const chaines = await this.fieldChainService.findMany({
      sourceName: Models.Table.Users,
      sourceId: id,
    });
