...
export class SelectAccessComponent implements OnInit {
  ...
  onChangeRoleDropdown() {
    this.selectedSources = this.availableAccessSources.filter(aas => this.gridData.find(d => d.sourceId === aas.id)).map(aas => aas.id);
  }

  getAvailableAccesses(sourceId: number) {
    return this.availableAccessSources.filter(aas => aas.id === sourceId || !this.selectedSources.includes(aas.id))
  }
}
