...
export class SettingsUsersComponent implements OnInit {
  ...
  gridActionsConfig: GridActionConfigItem<ServerUser.Response>[] = [
    {
      ...
      disabled: (user) => {
        ...
        const customRole = this.roles.find(
          (role) => role.id + 1000 === user.roleLevel,
        );

        return !(
          this.sessionService.isAdminOrHigher ||
          (this.sessionService.isContactCenterChief &&
            customRole?.systemName === ServerRole.Custom.contactCenter)
        );
      },
  ...
  ngOnInit(): void {
    zip(
      this.userService.getUserFields(),
    ...
    this.gridConfig = [
      ...
      },
      {
        ...
        getValue: (item) =>
          FieldsUtils.getFieldValue(item, FieldNames.User.name),
      },
      {
        title: this.strings.callRequestSources,
        name: FieldNames.User.callRequestSources,
        getValue: (item) =>
          FieldsUtils.getFieldValue(item, FieldNames.User.callRequestSources),
      },
      {
        title: this.strings.number,
        name: FieldNames.User.number,
        getValue: (item) =>
          FieldsUtils.getFieldValue(item, FieldNames.User.number),
      },
      {
  ...
  private getUsers() {
    ...
    return this.userService.getUsers().pipe(
      ...
      tap((result) => {
        ...
        const contactCenterRole = this.roles.find(
        ...
        this.rawUsers = this.sessionService.isContactCenterChief
          ? result.filter(
              (user: { roleLevel: any }) =>
                user.roleLevel === contactCenterRole.id + 1000,
            )
          : result;
        this.sortUsers();
      }),
  ...
  getRoleName(roleLevel: number): string {
    switch (roleLevel) {
      case ServerRole.System.SuperAdmin:
        return 'Супер Админ';
      case ServerRole.System.Admin:
        return 'Админ';
      case ServerRole.System.None:
        return 'Не назначена';
    }
    ...
    if (customRole) {
      return (
        (settingsUsersStrings as StringHash)[customRole.systemName] ||
        `${roleLevel}`
      );
    }
