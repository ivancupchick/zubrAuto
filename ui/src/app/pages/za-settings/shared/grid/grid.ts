...
export function getGridFieldsCompare<GridItemType extends { id: number }>(
  gridConfig: GridConfigItem<GridItemType>,
  event: SortEvent,
) {
  return (data1: GridItemType, data2: GridItemType) => {
    ...
    let value1 =
      gridConfig.isDate && v1 ? +moment(v1, 'DD.MM.YYYY HH:mm').toDate() : v1;
    let value2 =
      gridConfig.isDate && v2 ? +moment(v2, 'DD.MM.YYYY HH:mm').toDate() : v2;
    let result = 1;
