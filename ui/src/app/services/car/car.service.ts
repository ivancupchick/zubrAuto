...
export class CarService {
  ...
  getCars(): Observable<BaseList<ServerCar.Response>> {
    return this.requestService.get<BaseList<ServerCar.Response>>(
      `${environment.serverUrl}/${API}/${Constants.API.CRUD}`,
    );
  }
  ...
  getCarsByQuery(query: StringHash): Observable<BaseList<ServerCar.Response>> {
    ...
    return this.requestService.get<BaseList<ServerCar.Response>>(
      `${environment.serverUrl}/${API}/${Constants.API.CRUD}?${queries}`,
    );
  }

  createCar(value: ServerCar.CreateRequest): Observable<ServerCar.IdResponse> {
    return this.requestService
      .post<ServerCar.IdResponse>(
        `${environment.serverUrl}/${API}/${Constants.API.CRUD}`,
        value,
      )
      .pipe(
  ...
  updateCar(
    value: ServerCar.UpdateRequest,
    id: number,
  ): Observable<ServerCar.IdResponse> {
    delete (value as any).id;
    return this.requestService
      .put<ServerCar.IdResponse>(
        `${environment.serverUrl}/${API}/${Constants.API.CRUD}/${id}`,
        value,
      )
      .pipe(
  ...
  deleteCars(ids: number[]): Observable<ServerCar.Response[]> {
    return this.requestService
      .post<
        ServerCar.Response[]
      >(`${environment.serverUrl}/${API}/${Constants.API.DELETE_CARS}`, { carIds: ids })
      .pipe(
        map((result) => {
          console.log(result);
  ...
  createCarsByLink(link: string, userId: number) {
    return this.requestService
      .post<any>(
        `${environment.serverUrl}/${API}/${Constants.API.CREATE_CARS_BY_LINK}`,
        { link, userId },
      )
      .pipe(
        map((result) => {
  ...
  createManualCars(payload: { specialist: number; cars: string[] }) {
    return this.requestService
      .post<any>(
        `${environment.serverUrl}/${API}/${Constants.API.CREATE_CARS_BY_MANAGER}`,
        { ...payload },
      )
      .pipe(
        map((result) => {
  ...
  uploadCarImages(
    id: number,
    files: File[],
    carImageMetadata: CarImageMetadata,
  ) {
    ...
    return this.requestService
      .post<
        any,
        FormData
      >(`${environment.serverUrl}/${API}/${Constants.API.IMAGES}`, formData)
      .pipe(
        map((result) => {
          console.log(result);
  ...
  uploadCarImage360(
    ...
    return this.requestService
      .post<
        any,
        FormData
      >(`${environment.serverUrl}/${API}/${Constants.API.IMAGE360}`, formData)
      .pipe(
        map((result) => {
          console.log(result);
  ...
  uploadCarStateImages(
    ...
    return this.requestService
      .post<
        any,
        FormData
      >(`${environment.serverUrl}/${API}/${Constants.API.STATE_IMAGES}`, formData)
      .pipe(
        map((result) => {
          console.log(result);
  ...
  deleteCarImage(carId: number, imageId: number): Observable<boolean> {
    return this.requestService
      .delete<any>(
        `${environment.serverUrl}/${API}/${Constants.API.IMAGES}/${carId}/${imageId}`,
      )
      .pipe(
        map((result) => {
          console.log(result);
  ...
  changeCarStatus(
    ...
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        const statusConfig = allFields.find(
          (field) => field.name === FieldNames.Car.status,
        );
        const commentConfig = allFields.find(
          (field) => field.name === FieldNames.Car.comment,
        );
        const dateOfLastStatusChangeConfig = allFields.find(
          (field) => field.name === FieldNames.Car.dateOfLastStatusChange,
        );
        const dateOfNextActionConfig = allFields.find(
          (field) => field.name === FieldNames.Car.dateOfNextAction,
        );
        const dateOfFirstStatusChangeConfig = allFields.find(
          (field) => field.name === FieldNames.Car.dateOfFirstStatusChange,
        );

        if (
          statusConfig &&
          commentConfig &&
          dateOfLastStatusChangeConfig &&
          dateOfNextActionConfig &&
          dateOfFirstStatusChangeConfig
        ) {
          const statusField = FieldsUtils.setDropdownValue(
            statusConfig,
            newStatus,
          );
          const commentField = FieldsUtils.setFieldValue(
            commentConfig,
            comment,
          );
          const dateOfLastStatusChangeField = FieldsUtils.setFieldValue(
            dateOfLastStatusChangeConfig,
            `${+new Date()}`,
          );
          const dateOfNextActionField =
            dateOfNextAction &&
            FieldsUtils.setFieldValue(dateOfNextActionConfig, dateOfNextAction);
          const dateOfFirstStatusChangeField = FieldsUtils.setFieldValue(
            dateOfFirstStatusChangeConfig,
            `${+new Date()}`,
          );
          ...
          const fields = [
            statusField,
            commentField,
            dateOfLastStatusChangeField,
          ];

          dateOfNextAction &&
            dateOfNextActionField &&
            fields.push(dateOfNextActionField);
  ...
  saveVideo(carId: number, link = '') {
    ...
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        const linkConfig = allFields.find(
          (field) => field.name === FieldNames.Car.linkToVideo,
        );
  ...
  saveOldWorksheet(carId: number, oldWorksheet = '') {
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        const oldWorksheetConfig = allFields.find(
          (field) => field.name === FieldNames.Car.oldWorksheet,
        );

        console.log(oldWorksheetConfig);

        if (oldWorksheetConfig) {
          const oldWorksheetField = FieldsUtils.setFieldValue(
            oldWorksheetConfig,
            oldWorksheet,
          );
  ...
  selectMainPhoto(carId: number, photoId: number) {
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        const mainPhotoIdConfig = allFields.find(
          (field) => field.name === FieldNames.Car.mainPhotoId,
        );

        if (mainPhotoIdConfig) {
          const statusField = FieldsUtils.setFieldValue(
            mainPhotoIdConfig,
            `${photoId}`,
          );
  ...
  transformToCarShooting(
    id: number,
    shootingDate: number,
    carShootingSpecialistId: number,
    comment = '',
  ) {
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        ...
        const shootingDateConfig = allFields.find(
          (field) => field.name === FieldNames.Car.shootingDate,
        );
        const carShootingSpecialistIdConfig = allFields.find(
          (field) => field.name === FieldNames.Car.carShootingSpecialistId,
        );
        const statusConfig = allFields.find(
          (field) => field.name === FieldNames.Car.status,
        );
        ...
        if (
          ...
          commentConfig &&
          shootingDateConfig &&
          carShootingSpecialistIdConfig
        ) {
          const shootingDateField = FieldsUtils.setFieldValue(
            shootingDateConfig,
            `${shootingDate}`,
          );
          const carShootingSpecialistIdField = FieldsUtils.setFieldValue(
            carShootingSpecialistIdConfig,
            `${carShootingSpecialistId}`,
          );
          const statusField = FieldsUtils.setDropdownValue(
            statusConfig,
            newStatus,
  ...
  editCarForm(id: number, carForm: RealCarForm) {
    return this.fieldService.getFieldsByDomain(FieldDomains.Car).pipe(
      concatMap((allFields) => {
        ...
        const worksheetConfig = allFields.find(
          (field) => field.name === FieldNames.Car.worksheet,
        );

        if (worksheetConfig) {
          const worksheetField = FieldsUtils.setFieldValue(
            worksheetConfig,
            form,
          );
  ...
  addCustomerCall(carId: number) {
    ...
    return this.requestService.post<any>(url, {}).pipe(
      map((result) => {
        console.log(result);
  ...
  addCustomerDiscount(carId: number, discount: number, amount: number) {
    ...
    return this.requestService.post<any>(url, { discount, amount }).pipe(
      map((result) => {
        console.log(result);
  ...
  createCarShowing(carId: number, showingContent: CarStatistic.ShowingContent) {
    ...
    return this.requestService.post<any>(url, { showingContent }).pipe(
      map((result) => {
        console.log(result);
  ...
  getShowingCarStatistic(carId: number) {
    ...
    return this.requestService.get<CarStatistic.CarShowingResponse[]>(url).pipe(
      map((result) => {
        console.log(result);
